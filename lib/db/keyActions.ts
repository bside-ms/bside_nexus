...
export type CreateKeyTypeInput = {
...
export const createKeyType = async (input: CreateKeyTypeInput) => {
    const [row] = await db
...
export const softDeleteKeyType = async (id: string) => {
    const [row] = await db.update(keysBaseTable).set({ deleteAt: new Date() }).where(eq(keysBaseTable.id, id)).returning();
    return row;
};

export const listKeyTypes = async () => {
    return db
        .select()
        .from(keysBaseTable)
        .where(isNull(keysBaseTable.deleteAt))
        .orderBy(asc(keysBaseTable.keyNr));
};

// Key Items
export type CreateKeyItemInput = {
    id: string; // uuid
    keyTypeId: string;
    seqNumber: number;
    comment?: string | null;
...
export const createKeyItem = async (input: CreateKeyItemInput) => {
    const [row] = await db
...
export const changeKeyItemStatus = async (
    ...
    if (status !== 'active') {
        const active = await db
            ...
            .where(
                and(
                    eq(keyAssignmentTable.keyItemId, keyItemId),
...
export const fillUpKeyItems = async (keyTypeId: string, targetQuantity: number) => {
    return db.transaction(async (tx) => {
        ...
        for (let i = 1; i <= targetQuantity; i++) {
            if (!have.has(i)) toCreate.push(i);
        }
        if (toCreate.length === 0) return [] as Array<number>;
        // create rows with generated ids by caller is preferred, but here we can generate simple uuid via database using gen_random_uuid() if available
        // fallback: simple JS uuid not available here; require callers to create single items or extend API later.
        // For now, insert without explicit id is not possible due to schema requiring id.
        // We thus return the seqNumbers that need creation; API layer should create items with UUIDs.
        return toCreate;
    });
};

// Ausgabe (Issuance)
export const issueKeys = async (
    userProfileId: string,
    keyItemIds: Array<string>,
    createdBy?: string | null,
) => {
    if (keyItemIds.length === 0) return [] as Array<string>; // no protocols
    ...
    const profile = await db
        ...
        .limit(1);
    if (!profile[0] || profile[0].deleted) throw new Error('UserProfile nicht gefunden oder gelöscht.');
    ...
    if (items.length !== keyItemIds.length) throw new Error('Einige Schlüssel wurden nicht gefunden.');
    const notActive = items.find((i) => i.status !== 'active');
    if (notActive) throw new Error('Ein Schlüssel ist nicht aktiv und kann nicht ausgegeben werden.');
    ...
    const actives = await db
        ...
        .from(keyAssignmentTable)
        .where(
            and(inArray(keyAssignmentTable.keyItemId, keyItemIds), isNull(keyAssignmentTable.returnedAt), isNull(keyAssignmentTable.lostAt)),
        );
    if (actives.length > 0) throw new Error('Mindestens ein Schlüssel ist bereits aktiv zugewiesen.');
...
export const returnKeys = async (assignmentIds: Array<string>, createdBy?: string | null) => {
    if (assignmentIds.length === 0) return [] as Array<string>;
    // Load assignments and ensure open (not returned, not lost)
    const assignments = await db
        .select()
        .from(keyAssignmentTable)
        .where(inArray(keyAssignmentTable.id, assignmentIds));
    if (assignments.length !== assignmentIds.length) throw new Error('Einige Zuweisungen wurden nicht gefunden.');
    const invalid = assignments.find((a) => a.returnedAt !== null || a.lostAt !== null);
    if (invalid) throw new Error('Mindestens eine Zuweisung ist bereits geschlossen/verloren.');
    ...
    await db.transaction(async (tx) => {
        for (let i = 0; i < assignmentIds.length; i += 4) {
            ...
            await tx
                .insert(keyProtocolsTable)
                .values({ id: protocolId, protocolType: 'return', userProfileId: assignments[0].userProfileId, createdBy: createdBy ?? null })
                .returning();
            protocolIds.push(protocolId);
            const now = new Date();
            await tx
...
// Verlust markieren
export const markLost = async (assignmentId: string) => {
    return db.transaction(async (tx) => {
        const aList = await tx
            .select()
            .from(keyAssignmentTable)
            .where(eq(keyAssignmentTable.id, assignmentId))
            .limit(1);
        const a = aList[0];
        if (!a) throw new Error('Zuweisung nicht gefunden.');
        if (a.returnedAt) throw new Error('Zuweisung bereits zurückgegeben.');
        if (a.lostAt) return a; // already lost
        const now = new Date();
...
export const reactivateFoundKeyItem = async (keyItemId: string) => {
    ...
    const active = await db
        ...
        .limit(1);
    if (active.length > 0) throw new Error('Schlüssel ist aktiv zugewiesen.');
