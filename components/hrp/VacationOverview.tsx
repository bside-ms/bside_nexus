...
export default function VacationOverview({ showDatePicker = true, initialDate, userId }: VacationOverviewProps): ReactElement {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            try {
                ...
                let url = `/api/hrp/vacation-overview?date=${dateString}`;
                if (userId) {
                    url += `&userId=${userId}`;
                }
        ...
        <Card className="w-full">
            ...
            <CardContent className="p-3 pt-0">
                {isLoading ? (
                    ...
                    <p className="text-sm text-muted-foreground text-center py-4">Fehler beim Laden der Urlaubsdaten.</p>
                ) : (
                    <div className="space-y-1 text-xs">
                        <div className="flex justify-between border-b pb-0.5 text-muted-foreground">
                            <span>Jahresanspruch:</span>
                            <span className="font-medium text-foreground">{details.annualEntitlement} Tage</span>
                        </div>

                        {!details.isCarryOverExpired && (
                            <div className="flex justify-between border-b pb-0.5 text-muted-foreground">
                                <span>Resturlaub {year - 1}:</span>
                                <span className="font-medium text-foreground">{details.carryOver} Tage</span>
                            </div>
                        )}
                        ...
                        <div className="flex justify-between border-b pb-0.5 text-muted-foreground">
                            ...
                            <span className="font-medium text-foreground">
                                {details.isCarryOverExpired ? details.taken.fromAnnual : details.taken.total} Tage
                            </span>
                        </div>

                        <div className="flex justify-between border-b pb-0.5 text-muted-foreground">
                            <span>Davon geplant:</span>
                            <span className="font-medium text-foreground">{details.planned} Tage</span>
                        </div>
